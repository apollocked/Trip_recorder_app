...
class _TodoPageState extends State<TodoPage> {
  ...
  void _add(String title) {
    ...
    _c.clear();
    setState(() => _items = [_PrepItem(title: title.trim(), category: _cat), ..._items]);
    _save();
  }
  void _toggle(int i) {
        ...
        builder: (ctx, setDlg) => AlertDialog(
          ...
          content: Column(mainAxisSize: MainAxisSize.min, children: [
            TextField(controller: _c, autofocus: true,
              decoration: InputDecoration(hintText: l10n.addItemHint, filled: true,
                fillColor: Theme.of(ctx).colorScheme.surfaceContainerHighest.withAlpha(80),
          ...
          actions: [
            ...
            FilledButton(onPressed: () { if (_c.text.trim().isNotEmpty) { _add(_c.text); Navigator.pop(ctx); } },
    ...
    return Scaffold(
      ...
      body: _items.isEmpty
          ? ListView(children: [
              ...
              EmptyState(icon: Icons.checklist_rounded, title: l10n.noItemsYet, subtitle: l10n.addItemHint,
                action: FilledButton.tonalIcon(onPressed: _showAddDialog,
                  icon: const Icon(Icons.add_rounded, size: 18), label: Text(l10n.addItem))),
            ])
          : Column(children: [
              Padding(padding: const EdgeInsets.fromLTRB(20, 12, 20, 4),
                child: ClipRRect(borderRadius: BorderRadius.circular(8),
                  child: LinearProgressIndicator(value: checked / _items.length,
                    minHeight: 6, backgroundColor: c.surfaceContainerHighest))),
              Padding(padding: const EdgeInsets.symmetric(horizontal: 20),
                child: Text(l10n.itemsChecked(checked, _items.length),
                  style: t.labelSmall?.copyWith(color: c.onSurfaceVariant))),
              const SizedBox(height: 8),
              Expanded(child: ListView(padding: const EdgeInsets.only(left: 16, right: 16, bottom: 88),
                children: _buildSections(map, c, t, l10n))),
            ]),
    );
  }

  List<Widget> _buildSections(Map<_Cat, List<_PrepItem>> map,
      ColorScheme c, TextTheme t, AppLocalizations l10n) {
    final r = <Widget>[];
    var idx = 0;
    for (final cat in _Cat.values) {
      ...
      r.add(Padding(padding: const EdgeInsets.only(left: 4, right: 4, top: 12, bottom: 4),
          ...
          Row(children: [
            ...
            Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 3),
              ...
              child: Text('$ck/${items.length}',
                style: t.labelSmall?.copyWith(fontWeight: FontWeight.bold, color: c.onPrimaryContainer))),
          ]),
          if (items.length > 1) ...[
            const SizedBox(height: 6),
            ClipRRect(borderRadius: BorderRadius.circular(4),
              child: LinearProgressIndicator(value: ck / items.length,
                minHeight: 4, backgroundColor: c.surfaceContainerHighest)),
          ],
        ]),
      ));
        ...
        r.add(Padding(padding: const EdgeInsets.only(bottom: 6),
          child: Dismissible(
            ...
            confirmDismiss: (_) => showConfirmationDialog(context: context,
              ...
              message: l10n.confirmDeleteMessage, icon: Icons.delete_rounded),
...
class _PrepItem {
  ...
  _PrepItem copyWith({bool? done}) => _PrepItem._(title: title, done: done ?? this.done, category: category, seq: _seq);
  _PrepItem._({required this.title, this.done = false, required this.category, required int seq}) : _seq = seq;

  factory _PrepItem.fromJson(Map<String, dynamic> json) {
    final ci = json['category'] as int? ?? 4;
    return _PrepItem(title: json['title'] as String, done: json['done'] as bool? ?? false,
      category: _Cat.values[ci.clamp(0, 4)]);
  }

  Map<String, dynamic> toJson() => {'title': title, 'done': done, 'category': category.index};
}
